...
TopologicalMetrics = extract_topolo(A3LNetwork, all_proteins, verbose = user_verbose)
...
rwr_dist = extract_by_rwr(Graph = A3LNetwork, start_nodes = all_proteins, nCores = 1, verbose = user_verbose)
dim(rwr_dist)
...
Dist_cluster = extract_cluster(A3LNetwork, all_proteins, dist_matrix = rwr_dist, Cluster_list = Protein_clusters, verbose = user_verbose)
dim(Dist_cluster)
```
...
Dist_signature = extract_by_sig(A3LNetwork, all_proteins, Signature_for_prostate_cancer, Gene_clusters, rwr_dist, verbose = user_verbose)
dim(Dist_signature)
```
